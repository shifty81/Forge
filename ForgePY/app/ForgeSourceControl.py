...
FORGE_SOURCE_VERSION = "FORGEPY-SOURCE-0.4.17"
...
def _remote_kind(name: str, url: str) -> str:
    ...
    internal_name = str(sc.get("defaultInternalGitRemote") or "forgepy-internal").strip().casefold()
    if low_name == internal_name or low_name == "forgepy-internal":
        return "internal"
    internal_root = str(sc.get("internalGitRoot") or "").strip()
    if internal_root:
        try:
            remote_path = Path(url).expanduser().resolve()
            if remote_path.is_relative_to(Path(internal_root).expanduser().resolve()):
                return "internal"
        except Exception:
            pass
...
def _default_remote_name(kind: str) -> str:
    ...
    if kind == "internal":
        return str(sc.get("defaultInternalGitRemote") or "forgepy-internal").strip() or "forgepy-internal"
...
def status(root: Path) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        ...
        "upstream": "", "remotes": [], "githubConfigured": False, "internalGitConfigured": False, "forgejoConfigured": False,
    ...
    out["internalGitConfigured"] = any(x["kind"] == "internal" for x in remotes)
...
def command(root: Path, action: str, *extra: str) -> subprocess.CompletedProcess[str]:
    ...
    if action == "push-internal":
        from ForgePYInternalGit import push_snapshot
    ...
    if action == "sync-primary":
        ...
        output.append("=== FORGEPY INTERNAL GIT ===\n" + internal.stdout)
...
def main(argv: Sequence[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="ForgePY Git/GitHub/Internal Git authority")
    ap.add_argument("action", choices=("status-json", "status", "review", "diff", "history", "branches", "remotes", "fetch-all", "pull-ff", "push", "push-github", "push-internal", "push-forgejo", "sync-primary", "sync-both", "init", "set-remote"))
