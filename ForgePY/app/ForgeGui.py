...
from VaultDriveIndex import (scan as vault_drive_scan, list_projects as vault_drive_projects, list_entries as vault_drive_entries, search_entries as vault_drive_search, latest_summary as vault_drive_latest_summary, lineage_groups as vault_drive_lineage_groups)
from VaultForgejo import server_status as forgejo_server_status
from ForgePYSourceControl import status as forgepy_source_status
...
from ForgePYInternalGit import status as internal_git_status, repository_path as internal_git_repository_path
...
class ForgeGui:
    ...
    def _build_source_control_tab(self, parent: Any) -> None:
        ...
        self._section_title(
            shell,
            "Source Control",
            "GitHub and ForgePY Internal Git are the primary authorities. Forgejo remains optional compatibility hosting.",
        )
        ...
        self.source_workspace_status_label = tk.Label(
            body, text="Loading source-control status…", bg=PANEL, fg=MUTED,
        ...
        self._command_category_list(shell, "ForgePY Internal Git", (
            ("Ensure Internal Git", "Create/bind the active project's local bare repository without touching working-tree files.", lambda: self._start_internal_git("ensure"), True),
            ("Push Internal Snapshot", "Push the current committed branch to ForgePY Internal Git. Uncommitted files are never committed automatically.", lambda: self._start_internal_git("push"), False),
            ("Internal History", "Show recent commits preserved in the active project's ForgePY Internal Git repository.", lambda: self._start_internal_git("history"), False),
            ("Open Internal Repository", "Open the active project's local bare repository folder.", lambda: open_path(internal_git_repository_path(self.contract.project_id)), False),
        ))
        self._command_category_list(shell, "GitHub", (
            ...
            ("Sync Primary Authorities", "Snapshot the current commit to ForgePY Internal Git and push that commit to GitHub.", lambda: self._start_builtin_source("sync-primary", [self.contract.project_id]), False),
            ("Configure GitHub Remote", "Add or update the GitHub remote for this working tree.", lambda: self._configure_source_remote("github"), False),
            ("Open GitHub", "Open the active project's GitHub repository.", self._open_active_github, False),
        ))
        self._command_category_list(shell, "Working Tree", (
            ("Open Project Source Control", "Open the project's detailed Source Control page inside Project Workspace.", lambda: (self._show_app_tab("Project Workspace"), self._show_page("Source Control")), True),
            ("Review Changes", "Show the current diff summary against HEAD.", lambda: self._start_builtin_source("review"), False),
            ("History", "Show recent working-tree and remote history.", lambda: self._start_builtin_source("history"), False),
            ("Remotes", "List and classify configured remotes.", lambda: self._start_builtin_source("remotes"), False),
        ))
        self._command_category_list(shell, "Forgejo Compatibility (Optional)", (
            ("Server Status", "Check the optional local Forgejo service. ForgePY Internal Git does not require it.", self._refresh_forgejo_status_async, True),
            ("Start Forgejo", "Start the optional configured Forgejo service.", lambda: self._start_builtin_forgejo("start"), False),
            ("Open Forgejo", "Open the optional Forgejo web interface.", self._forgejo_open_web, False),
            ("Configure Forgejo Remote", "Bind a separate optional Forgejo remote to the active project.", lambda: self._configure_source_remote("forgejo"), False),
        ))
        self.forgejo_status_label = tk.Label(
            shell, text="Forgejo compatibility status not checked.", bg=BG, fg=MUTED,
            font=("Consolas", 8), anchor="w", justify="left",
        )
        self.forgejo_status_label.pack(fill="x", pady=(6, 0))
    ...
    def _render_health_gauge(self, health: Any) -> None:
        ...
        try:
            ...
            internal_state = "ready" if source.get("internalGitConfigured") else "setup needed"
            github_state = "ready" if source.get("githubConfigured") else "setup needed"
            self.right_context_label.configure(
                text=(f"{project_meta.get('version') or project_meta.get('build') or self.contract.kind}\n"
                      f"Branch: {git.get('branch') or '—'}\n"
                      f"GitHub: {github_state}\nInternal Git: {internal_state}\n"
    ...
    def _settings_build_pages(self) -> None:
        ...
        self._setting_check(p,"services.forgejoAutoStart","Start optional Forgejo with ForgePY",bool(svc.get("forgejoAutoStart",False)),"Optional compatibility hosting; ForgePY Internal Git works without Forgejo.")
        ...
        self._command_category_list(self._settings_pages["Services"],"Service Controls",(("Scan Intake Now","Run the intake classifier immediately.",self._vault_scan_intake,True),("Source Control","Open GitHub / Internal Git authority.",lambda:self._show_app_tab("Source Control"),False),("Cortex Status","Open Cortex integration status.",lambda:self._show_app_tab("Cortex"),False)))
        ...
        self._setting_entry(p,"sourceControl.gitBinary","Git binary",str(sc.get("gitBinary") or "")); self._setting_entry(p,"sourceControl.githubCliBinary","GitHub CLI (gh)",str(sc.get("githubCliBinary") or "")); self._setting_entry(p,"sourceControl.defaultGitHubRemote","GitHub remote",str(sc.get("defaultGitHubRemote") or "origin")); self._setting_check(p,"sourceControl.internalGitEnabled","Enable ForgePY Internal Git",bool(sc.get("internalGitEnabled",True)),"Local bare repository authority used alongside GitHub."); self._setting_entry(p,"sourceControl.internalGitRoot","Internal Git root",str(sc.get("internalGitRoot") or "")); self._setting_entry(p,"sourceControl.defaultInternalGitRemote","Internal Git remote",str(sc.get("defaultInternalGitRemote") or "forgepy-internal")); self._setting_check(p,"sourceControl.fetchOnStatus","Fetch remotes during status",bool(sc.get("fetchOnStatus",False)))
    ...
    def _vault_show_unclassified(self) -> None:
        """Show the drive items ForgePY still cannot place confidently."""
        self._vault_browser_mode = "drive"
        ...
        rows = vault_drive_entries(root, classification="UNKNOWN", limit=7500)
        top = "vault-unclassified"
        self.vault_tree.insert("", "end", iid=top, text="Unclassified drive content", values=("REVIEW", f"{len(rows)} item(s)", str(root)), open=True)
        for index, item in enumerate(rows):
            path = Path(str(item.get("path") or ""))
            iid = f"unclassified:{index}"
        ...
        self._append_log(f"[INFO] Vault unclassified review: {len(rows)} item(s) need ownership/classification decisions.\n", "info")
    ...
    def _vault_refresh_drive_tree(self) -> None:
        if not hasattr(self, "vault_tree"):
            return
        for iid in self.vault_tree.get_children():
            self.vault_tree.delete(iid)
        ...
        rows = vault_drive_entries(scan_root if summary else None, limit=7500) if summary else []
        if not rows:
            iid = "drive-empty"
            ...
            return
        root_iid = "drive-root"
        self.vault_tree.insert("", "end", iid=root_iid, text=str(scan_root), values=("VAULT_DRIVE", f"{len(rows)} shown", ""), open=True)
        groups: dict[str, str] = {}
        counts: dict[str, int] = {}
        for row in rows:
            ...
            iid = f"drive:{len(self._vault_drive_records)}"
            modified = ""
            try:
    ...
    def _refresh_source_status_async(self) -> None:
        if not hasattr(self, "source_status_label"):
            return
        self.source_status_label.configure(text="Checking Git / GitHub / ForgePY Internal Git…", fg=MUTED)
        root = self.root_path
        def work() -> None:
            try:
    ...
    def _refresh_source_workspace_async(self) -> None:
        if not hasattr(self, "source_workspace_status_label"):
            return
        self.source_workspace_status_label.configure(text="Checking GitHub / ForgePY Internal Git…", fg=MUTED)
        root = self.root_path
        project_id = self.contract.project_id
        def work() -> None:
            try:
                source = forgepy_source_status(root)
                internal = internal_git_status(root, project_id)
                self._event_q.put(("source-workspace-status", (source, internal)))
            except Exception as exc:
    ...
    def _start_internal_git(self, action: str) -> None:
        script = Path(__file__).resolve().parent / "ForgePYInternalGit.py"
        argv = [sys.executable, str(script), action, "--root", str(self.root_path), "--project-id", self.contract.project_id]
        self._start_builtin_argv(argv, label=f"internal-git-{action}", cwd=self.root_path, stay_on_tab=True)
    ...
    def _build_source_page(self, parent: Any) -> None:
        self._section_title(parent, "Source Control", "One Git working tree with GitHub and ForgePY Internal Git as the primary source authorities.")
        ...
        self._command_category_list(parent, "ForgePY Internal Git", (
            ("Ensure Internal Git", "Create/bind this project's local ForgePY bare repository without modifying working-tree files.", lambda: self._start_internal_git("ensure"), True),
            ("Push Internal Snapshot", "Preserve the current committed branch in ForgePY Internal Git.", lambda: self._start_internal_git("push"), False),
            ("Sync Primary", "Push the same current commit to ForgePY Internal Git and GitHub.", lambda: self._start_builtin_source("sync-primary", [self.contract.project_id]), False),
            ("Internal History", "Show history stored in the project's ForgePY Internal Git repository.", lambda: self._start_internal_git("history"), False),
        ))
        ...
        self._command_category_list(parent, "Branches / History", (
            ...
            ("Remotes", "List fetch/push URLs and verify GitHub/Internal/optional Forgejo classification.", lambda: self._start_builtin_source("remotes"), False),
    ...
    def _render_status(self, status: dict[str, Any]) -> None:
        ...
        lines = [
            ...
            f"Internal   : {'Configured' if source_control.get('internalGitConfigured') else 'Needs setup'}",
            f"GREEN      : {green_text}",
            f"Updates    : {upd_text}",
                ...
                if kind == "log":
                    ...
                    if hasattr(self, "source_status_label"):
                        if not info.get("gitReady"):
                            ...
                            internal = sorted({str(x.get("name")) for x in remotes if x.get("kind") == "internal"})
                            ...
                            self.source_status_label.configure(
                                text=(f"Branch {info.get('branch') or '<detached>'} @ {info.get('headShort') or '<no commit>'}  ·  "
                                      ...
                                      f"GitHub: {', '.join(gh) if gh else 'not configured'}  ·  Internal: {', '.join(internal) if internal else 'not configured'}"
                                      + (f"  ·  Forgejo(opt): {', '.join(fj)}" if fj else "")),
                                fg=GREEN if not dirty else YELLOW,
                            )
                elif kind == "source-workspace-status":
                    source, internal = payload
                    if hasattr(self, "source_workspace_status_label"):
                        ...
                        ig = "ready" if internal.get("repositoryReady") and internal.get("remoteConfigured") else ("repository ready / remote missing" if internal.get("repositoryReady") else "not initialized")
                        self.source_workspace_status_label.configure(
                            text=(f"{self.contract.name}  ·  {source.get('branch') or '<no branch>'} @ {source.get('headShort') or '<no commit>'}  ·  "
                                  ...
                                  f"GitHub: {gh}  ·  ForgePY Internal Git: {ig}\n{internal.get('repository') or ''}"),
                            fg=GREEN if not dirty and source.get("githubConfigured") and internal.get("remoteConfigured") else YELLOW,
                        )
    ...
    def _open_artifact_central_browser(self) -> None:
        ...
        def refresh(_e=None):
            ...
            for index,(project,category,path,size,mtime) in enumerate(data):
                iid=f"artifact:{index}"; path_map[iid]=path
        ...
        refresh(); self._center_modal(dialog, 1080, 650); self._round_window(dialog); dialog.deiconify(); dialog.lift(); dialog.grab_set(); entry.focus_force()
    ...
    def _open_patch_review(self) -> None:
        ...
        tree = self.ttk.Treeview(tree_frame, columns=("project","state","reason","received"), show="tree headings", selectmode="browse")
        ...
        def queue_selected(apply_now: bool = False) -> None:
            ...
            if apply_now:
                ...
                if (root / "app" / "ForgePYVersion.py").is_file():
                    ...
                    self.window.after(120, lambda: self._start_command("patch-apply", ["--yes"], label="review-apply"))
                return
            refresh()
        ...
        def archive_selected() -> None:
            item=selected_item()
            if not item: return
            if not self._popup("Archive to Patch Lineage", f"Move {item.get('source_name')} from actionable Review into historical Patch Lineage?", kind="warning", confirm=True):
                return
            try: vault_archive_review_item(str(item.get("intake_id") or ""))
            except Exception as exc: self._popup("Patch Review", str(exc), kind="error")
            refresh()
