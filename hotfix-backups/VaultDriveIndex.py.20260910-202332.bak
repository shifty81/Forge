...
def _connect() -> sqlite3.Connection:
    ...
    # Forward-migrate older drive_index.db project tables.
    cols = {str(row[1]) for row in db.execute("PRAGMA table_info(projects)").fetchall()}
    for name, decl in (("family_hint", "TEXT NOT NULL DEFAULT ''"), ("strength", "INTEGER NOT NULL DEFAULT 0")):
        if name not in cols:
            db.execute(f"ALTER TABLE projects ADD COLUMN {name} {decl}")
    db.commit()
    return db
...
def list_entries(scan_root: Path | None = None, *, classification: str | None = None, owner_project_root: Path | None = None, limit: int = 5000) -> list[dict[str, Any]]:
    if not db_path().is_file():
        return []
    db = _connect()
    try:
        ...
        if where:
            sql += " WHERE " + " AND ".join(where)
        sql += " ORDER BY path LIMIT ?"; args.append(max(1, int(limit)))
        rows = db.execute(sql, args).fetchall()
    finally:
        db.close()
...
def search_entries(query: str, *, scan_root: Path | None = None, limit: int = 1000) -> list[dict[str, Any]]:
    q = str(query or "").strip()
    if not q:
        return list_entries(scan_root, limit=limit)
    if not db_path().is_file():
        return []
    db = _connect()
    try:
